...
class TransaksiRequestController extends Controller
{
    ...
    public function updatescanqrCode(Request $request, $id_req) {
        ...
        // Ambil data transaksi berdasarkan id_req
        $transaksirequests = TransaksiRequest::where('id_req', $id_req)->first();
        ...
        // Jika semua field (date_arrived, date_completed, date_loading_goods, date_checkout_security) sudah terisi
        ...
        $status = 'CI SECURITY';  // Status default
    
        // Cek apakah ketiga field (date_arrived, date_completed, date_loading_goods) sudah terisi
        if ($transaksirequests->date_arrived && $transaksirequests->date_completed && $transaksirequests->date_loading_goods) {
            // Update status menjadi 'CO SECURITY' jika ketiga field terisi
            $status = 'CO SECURITY';
    
            // Update field date_checkout_security dengan waktu sekarang
            $transaksirequests->update([
                'status' => $status,
        ...
        // Jika status belum 'CO SECURITY' dan date_arrived sudah diisi, validasi
        if ($transaksirequests->date_arrived) {
            return response()->json([
                'success' => false,
                'message' => 'Date CI Security is already set and cannot be updated',
        ...
        // Update data transaksi jika status tetap 'CI SECURITY'
        $transaksirequests->update([
            'status' => $status,
            'date_arrived' => now(),
        ]);
...
//         return response()->json([
//             'success' => false,
//             'message' => 'Date Check In Security is already set and cannot be updated',
//         ], 400);
//     }
 
//     $transaksirequests->update([
//         'status' => 'CHECK IN INBOUND',
//         'date_loading_goods' => $request->date_loading_goods,
//         'date_arrived' => now(),
//     ]);
...
//             case 'CHECK IN INBOUND':
//                 if ($transaksiRequest->status !== 'CHECK IN SECURITY') {
//                     return response()->json([
//                         'success' => false,
//                         'message' => '',
//                     ], 400);
...
//             case 'CHECK OUT INBOUND':
//                 if ($transaksiRequest->status !== 'CHECK IN INBOUND') {
//                     return response()->json([
//                         'success' => false,
//                         'message' => 'Onl',
//                     ], 400);
